...
**纯后量子算法类**
- `SDF_ExportPublicKey_PQC` - 导出内部 PQC 公钥
- `SDF_InternalSign_PQC` / `SDF_InternalVerify_PQC` / `SDF_ExternalVerify_PQC` - ML-DSA/AIGIS 签名验签接口
- `SDF_GenerateKeyWithIPK_PQC` / `SDF_GenerateKeyWithEPK_PQC` / `SDF_ImportKeyWithISK_PQC` - ML-KEM 会话密钥封装与导入接口
...
- **纯后量子算法**：PQC 公钥导出、ML-DSA 签名验签、ML-KEM 封装导入
