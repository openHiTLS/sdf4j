...
JNIEXPORT jbyteArray JNICALL JNI_SDF_ExternalPrivateKeyOperation_RSA(JNIEnv *env, jobject obj, jlong sessionHandle,
    ...
    jbyte *input_buf = (*env)->GetByteArrayElements(env, dataInput, NULL);
    if (input_buf == NULL) {
    ...
    if (output_len == 0 || output_len > RSAref_MAX_LEN) {
        (*env)->ReleaseByteArrayElements(env, dataInput, input_buf, JNI_ABORT);
...
JNIEXPORT jbyteArray JNICALL JNI_SDF_ExternalDecrypt_ECC(JNIEnv *env, jobject obj, jlong sessionHandle, jint algID,
    ...
    if (ecc_cipher == NULL) {
        THROW_SDF_EXCEPTION(env, SDR_INARGERR, "Failed to convert cipher");
        return NULL;
    }
...
JNIEXPORT jbyteArray JNICALL JNI_SDF_ExternalKeyDecrypt(JNIEnv *env, jobject obj, jlong sessionHandle, jint algID,
    ...
    if (enc_buf == NULL) {
        (*env)->ReleaseByteArrayElements(env, key, key_buf, JNI_ABORT);
        if (iv_buf) (*env)->ReleaseByteArrayElements(env, iv, iv_buf, JNI_ABORT);
        THROW_SDF_EXCEPTION(env, SDR_NOBUFFER, "Memory allocation failed");
        return NULL;
    }
